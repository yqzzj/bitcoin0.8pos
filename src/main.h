...
/** for pos */
static const int64 IPO_SHARES = 3000000 * COIN; // Total number of shares to create using proof of work (intented for IPO)
static const int64 PROOF_OF_WORK_BLOCKS = 3000; // Block height of the last proof of work block
static const int STAKE_TARGET_SPACING = 60 * 1; // for pos: 1 minute block spacing
static const int STAKE_MIN_AGE = 60 * 60 * 8; // for pos: Minimum age for coin age changed to 8 minuts
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 90; // for pos: Stake age of full weight
...
public:
    ...
    {
        // for pos: the coin stake transaction is marked with the first output empty
    ...
    */
    bool AreInputsStandard(MapPrevTx &mapInputs) const;
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // for pos: get transaction coin age
...
public:
    ...
    // for pos: block signature - signed by coin base txout[0]'s owner
    ...
    // for pos: entropy bit for stake modifier if chosen by modifier
    ...
    // for pos: two types of block: proof-of-work or proof-of-stake
    ...
    // for pos: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // for pos: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // for pos: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // for pos: block index flags
    enum
    {
