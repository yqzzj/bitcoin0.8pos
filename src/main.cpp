...
static const int64 nTargetSpacingWorkMax = 12 * STAKE_TARGET_SPACING;
...
//
bool CTransaction::AreInputsStandard(MapPrevTx& mapInputs) const
{
    if (IsCoinBase())
...
bool CTransaction::ConnectInputs(CTxDB &txdb, MapPrevTx inputs, std::map<uint256, CTxIndex> &mapTestPool, const CDiskTxPos &posThisTx, const CBlockIndex *pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
...
//                return DoS(100, error("ConnectInputs() : %s prevout.n out of range %d %d %d prev tx %s\n%s", GetHash().ToString().substr(0,10).c_str(), prevout.n, txPrev.vout.size(), txindex.vSpent.size(), prevout.hash.ToString().substr(0,10).c_str(), txPrev.ToString().c_str()));
            ...
            // for pos: check transaction timestamp
...
//                return DoS(100, error("ConnectInputs() : txin values out of range"));
        ...
        if (IsCoinStake())
        {
            // for pos: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // for pos: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, bool fProofOfStake)
{
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // for pos: target change every block
    // for pos: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        CValidationState state;
        if (ProcessBlock(pfrom, &block) || state.CorruptionPossible())
            mapAlreadyAskedFor.erase(inv);
        int nDoS = 0;
        if (state.IsInvalid(nDoS))
            if (nDoS > 0)
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet *pwallet, bool fProofOfStake)
{
    ...
    // for pos: if coinstake available add coinstake tx
...
void static BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    try { while(true) {
        ...
        if (fProofOfStake)
        {
            // for pos: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinMiner, pwallet, fProofOfPos));
}

void GeneratePos(bool fGenerate, CWallet *pwallet)
{
    static boost::thread_group* minerThreads = NULL;
    bool fProofOfPos = true;
...
{
    return IPO_SHARES / PROOF_OF_WORK_BLOCKS;
}
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // for pos: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // for pos: track money supply and mint amount info
    ...
    // for pos: fees are not collected by miners as in bitcoin
    // for pos: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // for pos: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // for pos: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // for pos: record proof-of-stake hash value
    ...
    // for pos: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // for pos: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // for pos: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // for pos: check that the block satisfies synchronized checkpoint
    ...
    // for pos: check pending sync-checkpoint
    Checkpoints::AcceptPendingSyncCheckpoint();

    return true;
}

// for pos: total coin age spent in block, in the unit of coin-days.
